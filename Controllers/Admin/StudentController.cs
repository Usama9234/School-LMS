    ...
    public class StudentController : Controller
    {
        ...
        public IActionResult AddStudent(StudentDetails obj)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (db.students.Any(c => c.StudentEmail == obj.StudentEmail)|| db.Teachers.Any(c => c.TeacherEmail == obj.StudentEmail) || db.Parents.Any(c => c.ParentEmail == obj.StudentEmail))
                {
                    ModelState.AddModelError("StudentEmail", "Email already exists.");
                    var classList1 = db.Class.ToList();
                    var model1 = new StudentDetails { ClassesList = classList1, };
                    model1.ClassesList = db.Class.ToList();
                    return View(model1);
                }
                if (db.students.Any(c => c.StudentRollNo == obj.StudentRollNo))
                {
                    ModelState.AddModelError("StudentRollNo", "Student with this Roll No already exists.");
                    var classList1 = db.Class.ToList();
                    var model1 = new StudentDetails { ClassesList = classList1, };
                    model1.ClassesList = db.Class.ToList();
                    return View(model1);
