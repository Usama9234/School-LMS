...
namespace School_Management_System.Controllers.Admin
{
    public class TeacherController : Controller
    {
        ...
        public IActionResult AddTeacher(TeacherDetails obj)
        {
            ...
            ModelState.Remove("AssignedSubjects");
            ModelState.Remove("StudentAttendances");
            ...
            if (ModelState.IsValid)
            {
                ...
                if (db.students.Any(c => c.StudentEmail == obj.TeacherEmail) || db.Teachers.Any(c => c.TeacherEmail == obj.TeacherEmail) || db.Parents.Any(c => c.ParentEmail == obj.TeacherEmail))
                {
        ...
        public IActionResult ViewAllStaffAttendance()
        {
            List<StaffAttendance> objList = new List<StaffAttendance>();
            var teacherList = db.StaffAttendances.Include(f => f.Teacher).ToList();

            // Use a HashSet to track unique classes
            HashSet<int> uniqueClassIds = new HashSet<int>();

            // Filter out duplicate teachers while creating objList
            foreach (var teacher in teacherList)
            {
                // Check if the teacherid is already added to the HashSet
                if (!uniqueClassIds.Contains(teacher.TeacherId))
                {
                    objList.Add(teacher);
                    uniqueClassIds.Add(teacher.TeacherId);
                }
            }
        ...
        public IActionResult ViewStaffAttendance(int id)
        {
            List<StaffAttendance> objList = new List<StaffAttendance>();

            // Fetch only Teacher belonging to the selected
            var teacherList = db.StaffAttendances.Include(f => f.Teacher)
        ...
        public IActionResult AddStaffAttendance()
        {
            var teacherList = db.Teachers.ToList();
        ...
        [HttpPost]
        public IActionResult AddStaffAttendance(StaffAttendance obj)
        {
            ...
            if (ModelState.IsValid)
            {
                obj.TeacherId = obj.TeacherId;
                obj.Teacher = obj.Teacher;
                obj.TeachersList = obj.TeachersList;
                var model1 = new StaffAttendance
                {
                    TeachersList = db.Teachers.ToList()
                };
                if (obj.TeacherId <= 0)
                {
                    TempData["error"] = "Please Select Teacher";
                    return View(model1);
                }

                var selectedTeacherName = teacherList.FirstOrDefault(t => t.TeacherId == obj.TeacherId)?.TeacherName;

                var newTeacherAttendance = new StaffAttendance
                {
                    TeacherId = obj.TeacherId,
                    Date = obj.Date,
                    Remarks = obj.Remarks,
                ...
                db.StaffAttendances.Add(newTeacherAttendance);

                db.SaveChanges();
                TempData["success"] = "Attendance Added Successfully";
                return RedirectToAction("ViewAllStaffAttendance");
            }
        ...
        public IActionResult EditStaffAttendance(int id)
        {
            // Fetch the assigned subject based on the id
            var assignedAttendance = db.StaffAttendances.FirstOrDefault(t => t.Id == id);
            ...
            {
                TeachersList = teacherList,
                Date = assignedAttendance.Date,
                Remarks = assignedAttendance.Remarks,
                teacherName = assignedAttendance.teacherName

            };
            HttpContext.Session.SetString("TeacherName", assignedAttendance.teacherName);
            HttpContext.Session.SetInt32("TeacherId", assignedAttendance.TeacherId);
            return View(model);
        }


        [HttpPost]
        public IActionResult EditStaffAttendance(StaffAttendance obj)
        {
            obj.TeacherId = HttpContext.Session.GetInt32("TeacherId") ?? 0;
            ...
            if (ModelState.IsValid)
            {
                ...
                var existingRecord = db.StaffAttendances.FirstOrDefault(f => f.Id == obj.Id);

                if (existingRecord != null)
                {
                    // If it exists, update the existing record
                    existingRecord.Date = obj.Date;
                    existingRecord.Remarks = obj.Remarks;
                    existingRecord.teacherName = HttpContext.Session.GetString("TeacherName");
                    existingRecord.TeacherId = HttpContext.Session.GetInt32("TeacherId") ?? 0;
